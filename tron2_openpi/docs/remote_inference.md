...
"Remote" here means another authorized system on a controlled robot LAN. Do
not expose the policy server, robot control, or Bridge observation interfaces to
...
policy server and robot-control links must not be treated as authenticated or
encrypted, and a `wss://` Bridge endpoint does not secure the other links. Any
Internet-facing, cross-site, or cloud topology requires a separate security
review before use. Source disclosure is not a functional safety approval or
real-robot certification.
