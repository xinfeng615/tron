...
The ALOHA source is an external checkout and is not included in this source
snapshot. Clone the approved source into the path expected by this example and
detach it at the pinned commit:

```bash
git clone https://github.com/Physical-Intelligence/aloha.git third_party/aloha
git -C third_party/aloha checkout --detach d1dc83afd89ded4379851257fe5d85632d31d5ec
```

The external checkout remains governed by its own license. Review that
repository's license and prerequisites before use. This snapshot does not
claim that the checkout, Docker flow, or real-robot example has been run.

This example uses the Physical Intelligence fork of the ALOHA repository, with
very minor modifications to use Realsense cameras.
