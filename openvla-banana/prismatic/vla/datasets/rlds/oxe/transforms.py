...
def tron2_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """Keep the deployed arms/grippers and drop the two non-controlled head dimensions."""
    trajectory["action"] = tf.cast(trajectory["action"][:, :16], tf.float32)
    trajectory["observation"]["state"] = tf.cast(
        trajectory["observation"]["state"][:, :16], tf.float32
    )
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "tron2_lerobot": tron2_dataset_transform,
