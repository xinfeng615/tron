...
def apply_frame_transforms(
    ...
    *,
    ...
    if train:
        ...
        def aug(frame: dict):
            aug_seed = tf.random.uniform([2], maxval=tf.dtypes.int32.max, dtype=tf.int32, seed=seed)
            aug_fn = partial(obs_transforms.augment, seed=aug_seed, augment_kwargs=image_augment_kwargs)
...
def make_interleaved_dataset(
    ...
    *,
    ...
    if seed is None:
        ...
        try:
            dataset = dl.DLataset.sample_from_datasets(datasets, sample_weights, seed=seed)
        except TypeError:
