...
class GenerateConfig:
    ...
    seed: int = 7
    action_chunk_ensemble: bool = False
    action_chunk_ensemble_decay: float = 0.8
    binarize_gripper: bool = True


def temporal_ensemble_action(chunk_history, step_idx: int, decay: float) -> np.ndarray:
    """Combine all chunk predictions that target the current environment step."""
    candidates, weights = [], []
    for start_step, action_chunk in chunk_history:
        offset = step_idx - start_step
        if 0 <= offset < len(action_chunk):
            candidates.append(action_chunk[offset])
            weights.append(np.exp(decay * offset))
    if not candidates:
        raise RuntimeError(f"No action-chunk candidate is available at step {step_idx}.")
    weights = np.asarray(weights, dtype=np.float32)
    weights /= np.sum(weights)
    return np.sum(np.asarray(candidates) * weights[:, None], axis=0)
...
def eval_m6(cfg: GenerateConfig) -> None:
    ...
    for task_name in M6_TASK_NAMES:
        env = MetaworldEnv(task_name, seed=cfg.seed)
        total_return = 0.0
        total_success = 0
        ...
        for episode_idx in tqdm(range(cfg.num_trials_per_task), desc=task_name):
            ...
            for step_idx in range(cfg.max_steps):
                ...
                if cfg.action_chunk_ensemble:
                    action_chunk = get_action_chunk(
                        cfg, model, observation, task_description, processor=processor
                    )
                    chunk_history.append((step_idx, action_chunk))
                    chunk_history = [
                        (start_step, chunk)
                        for start_step, chunk in chunk_history
                        if step_idx - start_step < len(chunk)
                    ]
                    action = temporal_ensemble_action(
                        chunk_history, step_idx, cfg.action_chunk_ensemble_decay
                    )
                else:
                    action = get_action(cfg, model, observation, task_description, processor=processor)
                action = normalize_gripper_action(action, binarize=cfg.binarize_gripper)
                if cfg.model_family == "openvla":
